<template>
  <section class="createPost-container">
      <el-form ref="roleForm"  label-width="125px" v-loading="loading" element-loading-text="如心动则人妄动，伤其身痛其骨，于是体会到世间诸般痛苦">
        ...
        <el-row class="createPost-main-container">
	          <el-form-item label="角色名称:" required :class="{error: validation.hasError('roleForm.name')}">
	            <el-input v-model="roleForm.name" style="width:300px;"></el-input>
              <span class="v-message">{{ validation.firstError('roleForm.name') }}</span>
	          </el-form-item>
	          ...
	          <el-form-item label="备注:">
	            <el-input type="textarea" v-model="roleForm.remark" style="width:500px;margin-bottom:0px;"></el-input>
	          </el-form-item>
...
<script>
  ...
  import {saveRole,getRoleDetail,getMenuList} from "../../api/api";
  export default {
    name:"roleDetail",
    components: { Sticky },
    data() {
    ...
    validators: {
      'roleForm.name': function(value) {
        return this.validator.value(value).required('请填写角色名称');
      },
